<template>
  <div class="mainFoods">
    <div class="addList col-sm-6 col-md-4">
      <div class="panel panel-success" v-for="(item, index) in addedItems" :key="index">
        <div class="panel-heading">
          <h3 class="panel-title" style="text-transform: capitalize;">{{item.NAME}} ({{item.QUANTITY*100}}g)</h3>
        </div>
    ...
    <div class="total">
      ...
      <div class="panel-body">
        <app-nutrient-box :nutrientArray="totalForToday" size="large"></app-nutrient-box>
      </div>
    </div>
  </div>
</template>
